...
OptimiseParam = function(theta, runMode = 'max') {
	...
	for (si in 1:length(unSeasonNumber)) {
		...
		for (gi in 2:currentMaxNumGame) {
			...
			overallMeanMinutePlayed = with(pastAppearanceDF, mean(minute[isStart]))
			byPlayerInfo = pastAppearanceDF %>%
								filter(isStart) %>%
								group_by(team, player) %>%
								summarise(sumMinute = sum(timeDownweight * minute),
											sumStart = sum(timeDownweight))
			
			### now we can make our probabilities
			byPlayerInfo$topLine = with(byPlayerInfo, priorStrength * overallMeanMinutePlayed + sumMinute)
			byPlayerInfo$bottomLine = with(byPlayerInfo, priorStrength + sumStart)
			byPlayerInfo$expectedMinute = with(byPlayerInfo, topLine / bottomLine)
			
			appearanceDF$isCurrentGW = with(appearanceDF, seasonNumber == si & teamgamenumber == gi)
			appearanceDF = subset_join(appearanceDF,
										byPlayerInfo %>%
										select(team, player, expectedMinute),
										c('team', 'player'),
										isCurrentGW)
		}
	...
	## CAREFUL have not looked at the rest of the code after this point
