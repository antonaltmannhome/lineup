### let's start with the shot-value-estimation file but try to look into things like the effect of the players position, and check we've ealt with set pieces properly
# hmm, don't think we can take set pieces out of the thing. can we get good estimates just using the up to date data instead...?
...
# let's build this up gradaully. this next line is BS because i've not factored in minutes played, by the way
mod = glm(nonPenaltyGoal ~ oosNonPenaltyGoal + oosOffTargetNotBar,
          family = poisson,
          data = subOosDF)
...
AccuracyLikFunct = function(theta) {
  ...
  subOosDF$eGoal = with(subOosDF, minute / 96 * (
    ...
    barValue * bar +
    offTargetNotBarValue * offTargetNotBar +
    blockValue * block
  ))
  subOosDF$logLik = with(subOosDF, dpois(nonPenaltyGoal, eGoal))
