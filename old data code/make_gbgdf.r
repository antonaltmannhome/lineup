...
ArchivePreviousSeason = function(currentseason) {
  ...
  for (si in which(seasoninfo$toarchive)) {
    StoreSeason(seasoninfo$season[si])
    message('Have archived all game by game data for ', seasoninfo$season[si])
  }
}

StoreSeason = function(myseason) {
  initialgbgdf = MakeInitialGbgDF(myseason, seasoninfo)
  gbgdf = RemoveObviousLeavingPlayer(initialgbgdf)$gbgdf
...
MakeCurrentPlayerFile = function(currentseason) {
  seasoninfo = read.csv(paste(DATAPATH,'seasoninfo.csv',sep=''))
  initialgbgdf = MakeInitialGbgDF(currentseason, seasoninfo)
  dum = RemoveObviousLeavingPlayer(initialgbgdf)
  gbgdf = dum$gbgdf
